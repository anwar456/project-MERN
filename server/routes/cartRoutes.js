...
router.post("/addToCart", auth, async (req, res, next) => {
  ...
  //   if customers cart already exists
  if (customerCart) {
    Cart.findOneAndUpdate(
      ...
      .then((data, error) => {
        ...
        if (data) {
          ...
          // if item doesn't exists in cart, push item to cart
          Cart.findOneAndUpdate(
            {
              _customerId: req.customerId,
            },
    ...
    //   if customer cart does not exists, add new customer cart
...
router.put("/updateCartItem", auth, async (req, res) => {
  ...
  Cart.findOneAndUpdate(
    {
      _customerId: req.customerId,
      "cartDetails._productId": _productId,
    },
...
router.put("/removeCartItem/:id", auth, async (req, res) => {
  ...
  Cart.findOneAndUpdate(
    ...
    .exec((error, data) => {
      ...
      return res.status(200).json({
        status: true,
        message: "Item in cart has been remove successfully!",
        data,
      });
    });
});
